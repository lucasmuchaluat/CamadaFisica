...
serialName = "COM14"                  # Windows(variacao de)
 ...
 #=======================================================
def add_head (type_msg, payload, total_of_packages, current_package, server_id, CRC):
...
#          tipo*1       +   server_id*1   +  size*1     +   current package*3   + total packages*3        + crc_num*2 +  null bytes*6 = 17 bytes
...
#  ):
	...
	def send_message_4 (current_package):
		headfile = add_head(4,0,0,current_package,0,0)
	...
	def send_message_6 (expected_package):
		headfile = add_head(6,0,0,expected_package,0,0)
	...
	while finish == False:
		...
		# tipo*1   +   server_id*1   +  size*1   +   current package*3   + total packages*3 + null bytes*2 = 11 bytes
				...
				if msg_type == 3:
					...
					total_of_packages = int.from_bytes(head[6:9], "big")
					CRC_received = int.from_bytes(head[9:11], "big")

					payload_eop, payload_size = com.getData(payload_size+3)
					CRC_calculated = CRC16().calculate(payload_eop[:-3])

					if CRC_calculated - CRC_received == 0:
						print("CRC conferido com sucesso!")
